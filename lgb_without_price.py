...
df_train = pd.read_table('totalExposureLog_count1_static_uniq_res_without_price.txt')
df_test = pd.read_csv('./test_sample_without_price.csv')
...
predictions['target'] = np.mean(predictions[[col for col in predictions.columns if col not in ['sample_id', 'label']]].values, axis=1)
# predictions.to_csv('lgb_all_predictions_no_price_01.csv', index=None)
sub_df = pd.DataFrame({'sample_id':df_test['sample_id'].values})
sub_df["target"] = predictions['target']
sub_df.to_csv("lgb_submission_with_price_size_id_log_CV3_no_price_no_product_type.csv", index=False)
